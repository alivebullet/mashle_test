...
-- Generated by scripts/bundle-lua.ps1
-- Entry: logger
...
reopenBtn.Text             = "ðŸ“‹ Show Detector"
...
animTabBtn.Text             = "ðŸŽ¬  Animations"
...
searchBox.Text = ""
searchBox.PlaceholderText = "ðŸ” Filter remotes..."
...
exportRemotesBtn.Text = "ðŸ“¤ Export"
...
exportAnimationsBtn.Text = "ðŸ“¤ Export"
...
local function updateColorPickerUI()
	...
	hueValueLabel.Text = ("%dÂ°"):format(math.floor((rangeHue * 360) + 0.5) % 360)
...
pauseAnimBtn.MouseButton1Click:Connect(function()
	...
	pauseAnimBtn.Text = animLogPaused and "â–¶ Resume" or "â¸ Pause"
	updateStatus()
end)

pauseRemotesBtn.MouseButton1Click:Connect(function()
	remoteLogPaused = not remoteLogPaused
	pauseRemotesBtn.Text = remoteLogPaused and "â–¶ Resume" or "â¸ Pause"
	updateStatus()
end)
...
refreshAnimBtn.Text = "ðŸ”„ Refresh"
...
rdTitleLabel.Text               = "ðŸ“¡  Remote Details"
...
local rdCopyCodeBtn = makeRdBtn("ðŸ“‹ Copy Code", Color3.fromRGB(48, 88, 150), 1, 4)
local rdCopyPathBtn = makeRdBtn("ðŸ”— Copy Path", Color3.fromRGB(40, 105, 70), 2, 4)
local rdRunCodeBtn  = makeRdBtn("â–¶ Run Code",  Color3.fromRGB(65, 105, 40), 3, 4)
local rdPauseBtn    = makeRdBtn("â¸ Pause",     Color3.fromRGB(130, 48, 48), 4, 4)
...
local function showRemoteDetail(data)
	...
	rdTitleLabel.Text = "ðŸ“¡  " .. (data.remoteName or "Remote")
...
rdCopyCodeBtn.MouseButton1Click:Connect(function()
	...
	if tryClipboard(code) then flashRd(rdCopyCodeBtn, "âœ” Copied!", "ðŸ“‹ Copy Code")
	else print("[RemoteSpy] Code:", code); flashRd(rdCopyCodeBtn, "Printed!", "ðŸ“‹ Copy Code") end
end)

rdCopyPathBtn.MouseButton1Click:Connect(function()
	if not selectedRemoteData then return end
	local path = getRemotePath(selectedRemoteData.remote)
	if tryClipboard(path) then flashRd(rdCopyPathBtn, "âœ” Copied!", "ðŸ”— Copy Path")
	else print("[RemoteSpy] Path:", path); flashRd(rdCopyPathBtn, "Printed!", "ðŸ”— Copy Path") end
end)
...
rdRunCodeBtn.MouseButton1Click:Connect(function()
	...
	if ok then
		flashRd(rdRunCodeBtn, "âœ” Done!", "â–¶ Run Code")
	else
		warn("[RemoteSpy] Runtime:", runErr)
		flashRd(rdRunCodeBtn, "âš  Error!", "â–¶ Run Code")
	end
end)
			...
			if ok and tracks then
				for _, t in ipairs(tracks) do
					table.insert(playingTracks,
						("  â€¢ %s  [%s]"):format(t.Name~=""and t.Name or "Unnamed",
...
local function addLogEntry(data)
	...
	top.Text = ("[%s] %s â€” %.1f studs"):format(data.timestamp, displayName, data.distance)
...
exportRemotesBtn.MouseButton1Click:Connect(function()
	...
	if tryClipboard(exportText) then
		flashRd(exportRemotesBtn, "âœ” Exported!", "ðŸ“¤ Export")
	else
		print("[RemoteSpy] Exported logs:\n" .. exportText)
		flashRd(exportRemotesBtn, "Printed!", "ðŸ“¤ Export")
	end
end)
...
exportAnimationsBtn.MouseButton1Click:Connect(function()
	...
	local exportText = table.concat(lines, "\n")
	if tryClipboard(exportText) then
		flashRd(exportAnimationsBtn, "âœ” Exported!", "ðŸ“¤ Export")
	else
		print("[AnimDetect] Exported logs:\n" .. exportText)
		flashRd(exportAnimationsBtn, "Printed!", "ðŸ“¤ Export")
	end
end)
...
local function setupRemoteSpy()
	...
	print("[RemoteSpy] __namecall hook active â€” remote logging enabled.")
end
